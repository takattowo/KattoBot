    ...
    static class CatBrain
    {
        ...
        internal static async Task WatchForQuickReconnect(SocketGuildUser user)
        {
            ...
            (new Thread(() =>
            {
                ...
                {
                    var rps = CatBrain.ReportSuspicious(guild, null, user, null, $"User has left the server and rejoined within {minutes} minute{(minutes > 1 ? "s" : "")}. Ban/Role evade attempt?");
                    rps.Wait();
                }
            })).Start();
        }
