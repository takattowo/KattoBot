...
:cat: **KATTO Bot Commands and help** :cat:
```
KATTO will only respond to users with an Administrator role.
Btw I am very horny >w<

> I am designed to back up and restore all the possible roles for users that join or leave the server~ Here are the commands that you will need:
katto.add_blacklisted_roles @role1 @role2 @role3
katto.remove_blacklisted_roles @role1 @role2 @role3
katto.clear_blacklisted_roles
katto.show_blacklisted_roles

> The report channel (disabled by default) is a channel where the bot will report suspicious activity such as:
...
katto.set_report_channel #channel_name
katto.disable_report_channel
katto.show_report_channel
katto.add_suspicious_words fries, ketchup, hot dog, double cheeseburger
katto.remove_suspicious_words bicycle, tennis, golf shoes, soccer ball
katto.clear_suspicious_words
katto.show_suspicious_words

>> You can ignore channels as well
katto.add_ignored_channels #channel1 #channel2 #channel3
katto.remove_ignored_channels #channel1 #channel2 #channel3
katto.clear_ignored_channels
katto.show_ignored_channels

```
");
        }

        internal static async Task HORNY(SocketCommandContext context)
        {
            await context.Channel.SendMessageAsync(@$"Nooooo don't take me to the horny realm D::::");
        }
        internal static async Task ACCESSDENIED(SocketCommandContext context)
        {
            await context.Channel.SendMessageAsync(@$"You don't have access to this command~");
        }
        internal static async Task HELPMEMBER(SocketCommandContext context)
        {
            await context.Channel.SendMessageAsync(@$":cat: **KATTO Bot Commands and help** :cat:
```
I am pretty useless let a lone horny, someone plis help ;-;
```");
        }
        ...
        {
            //handle user left
            await CatBrain.WatchForQuickReconnect(user);
        ...
        {
            //handle user joined
            await CatBrain.RestoreRoles(user);
        }
        ...
        internal static async Task MessageDeleted(Discord.Cacheable<Discord.IMessage, ulong> cachedMsg, ISocketMessageChannel channel)
        {
            if (cachedMsg.HasValue)
            {
                ...
                if (imsg is SocketUserMessage msg)
                {
                    ...
                    bool suspicious = await CatBrain.EvaluateSuspiciousness(context, msg);

                    if (suspicious)
                    {
                        await CatBrain.ReportSuspicious(context, msg, "Suspicious message was deleted");
                    }
